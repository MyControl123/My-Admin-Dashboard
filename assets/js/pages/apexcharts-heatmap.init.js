function getChartColorsArray(a) {
  if ((console.log(a), null !== document.getElementById(a))) {
    var e = document.getElementById(a).getAttribute("data-colors");
...
var chartBarColors = getChartColorsArray("basic_heatmap"),
  options = {
    ...
    title: { text: "HeatMap Chart (Single color)", style: { fontWeight: 500 } },
  },
  chart = new ApexCharts(document.querySelector("#basic_heatmap"), options);
function generateData(a, e) {
  for (var t = 0, r = []; t < a; ) {
    var n = (t + 1).toString(),
      m = Math.floor(Math.random() * (e.max - e.min + 1)) + e.min;
...
colors.reverse();
(chartBarColors = getChartColorsArray("multiple_heatmap")),
  (options = {
    series: data,
    chart: { height: 450, type: "heatmap", toolbar: { show: !1 } },
    dataLabels: { enabled: !1 },
...
(chartBarColors = getChartColorsArray("color_heatmap")),
  (options = {
    series: [
      ...
      heatmap: {
        ...
        colorScale: {
          ranges: [
            ...
            { from: 6, to: 20, name: "Medium", color: "#51d28c" },
            { from: 21, to: 45, name: "High", color: "#564ab1" },
            { from: 46, to: 55, name: "Extreme", color: "#f7cc53" },
          ],
        },
      },
    },
...
(chartBarColors = getChartColorsArray("shades_heatmap")),
  (options = {
    series: [
    ...
    plotOptions: {
      heatmap: {
        ...
        colorScale: {
          ranges: [
            { from: 0, to: 50, color: "#038edc" },
            { from: 51, to: 100, color: "#5fd0f3" },
          ],
        },
      },
    },
