  ...
  if ((console.log(e), null !== document.getElementById(e))) {
    ...
    return (t = JSON.parse(t)).map(function (e) {
      ...
      return 2 != n.length
        ? t
        : "rgba(" +
            getComputedStyle(document.documentElement).getPropertyValue(n[0]) +
            "," +
            n[1] +
            ")";
    });
...
var chartBarColors = getChartColorsArray("basic_candlestick"),
  options = {
    ...
    plotOptions: {
      candlestick: {
        colors: { upward: chartBarColors[0], downward: chartBarColors[1] },
      },
    },
    title: {
  ...
  chart = new ApexCharts(document.querySelector("#basic_candlestick"), options);
chart.render();
(chartBarColors = getChartColorsArray("combo_candlestick")),
  (options = {
...
var chartBarColors = getChartColorsArray("combo_candlestick_chart"),
  optionsBar = {
    ...
    chart: {
      height: 150,
      type: "bar",
      brush: { enabled: !0, target: "candles" },
      selection: {
        enabled: !0,
        xaxis: {
    ...
    plotOptions: {
      bar: {
        ...
        colors: {
          ranges: [
            { from: -1e3, to: 0, color: "#f1734f" },
            { from: 1, to: 1e4, color: "#f7cc53" },
          ],
        },
      },
    },
...
chartBar.render();
(chartBarColors = getChartColorsArray("category_candlestick")),
  (options = {
    series: [
      {
        name: "candle",
